...
document.querySelectorAll(".benefit-card").forEach((card) => {
  card.addEventListener("click", () => {
    ...
    benefitDetailContent.innerHTML = `<h4 class="text-lg font-semibold text-white mb-2">${data.title}</h4><p class="text-aurora-gray text-[14px] leading-[1.7]">${data.text}</p>`;
...
document.querySelectorAll(".industry-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    document.querySelectorAll(".industry-tab").forEach((t) => {
      t.classList.remove("bg-gradient-to-r", "from-aurora-green", "to-aurora-blue", "text-white", "active");
      t.classList.add("bg-white/5", "text-aurora-gray");
    });
    // Activate clicked tab
    tab.classList.add("bg-gradient-to-r", "from-aurora-green", "to-aurora-blue", "text-white", "active");
    tab.classList.remove("bg-white/5", "text-aurora-gray");
...
window.addEventListener("scroll", () => {
  ...
  if (currentScroll > 100) {
    navbar.classList.add("bg-aurora-navy/95", "shadow-lg");
  } else {
    navbar.classList.remove("bg-aurora-navy/95", "shadow-lg");
  }
